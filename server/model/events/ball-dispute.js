...
function calculateTackleProbability(attackingPlayer, defendingPlayer) {
    const baseTackleChance = defendingPlayer.tackle / 100;
    const ballProtection = attackingPlayer.ballProtection / 100;    
    const tackleSuccessChance = ((baseTackleChance / (baseTackleChance + ballProtection)) * 100) + (Math.random() * 10 - 5);
...
export const detectBallDisputeEvent = (game) => {
    ...
    if (nearbyPlayers.length >= 2 && teamsNearBall.size > 1) {
        ...
        const tacklingPlayers = nearbyPlayers.filter(player => player.teamId !== playerWithBall.teamId);
        ...
        const successfulTackle = tackleProbabilities.find(tackle => randomNumber <= tackle.probability);
        const goalsBeforeEvent = nearbyPlayers.map(player => player.shortTermGoal);
        if (successfulTackle) {
        ...
        return new Event("BALL_DISPUTE", nearbyPlayers, goalsBeforeEvent, nearbyPlayers.map(player => player.shortTermGoal), playerWithBall.hasBall ? "FAIL" : "SUCCESS");
    }
}