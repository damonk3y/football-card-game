...
socket.on("new_game_started", (data) => {
    ...
    data.teams[1].players.forEach(player => {
        const parsedCoordinates = convertServerCoordinatesToPaper(player.x, player.y);
        ITEM_MAP.set(player.id, drawPlayer(parsedCoordinates.x, parsedCoordinates.y, player.shirtNumber, data.teams[1].color));
    });
});
...
socket.on("game_tick", (data) => {
    ...
    Object.keys(ballItems).forEach(key => {
        ballItems[key].position = new Point(convertServerCoordinatesToPaper(data.ball.x, data.ball.y));
    });
    data.teams[0].players.forEach(player => {
        const playerItems = ITEM_MAP.get(player.id);
        Object.keys(playerItems).forEach(key => {
            const parsedCoordinates = convertServerCoordinatesToPaper(player.x, player.y);
            playerItems[key].position = new Point(parsedCoordinates.x, parsedCoordinates.y);
        });
    });
    data.teams[1].players.forEach(player => {
        const playerItems = ITEM_MAP.get(player.id);
